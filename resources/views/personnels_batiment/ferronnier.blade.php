...
<body class="box-border bg-blue-50">
    ...
    <section class="max-h-full px-44 mx-44">
        <div>
            <form action="{{url('/personnels/ferronniers/search')}}" method="Get">
                @csrf
                <div class="flex justify-between pb-4 pt-4">
                    <input class="rounded-sm pl-2 h-10 w-2/3 placeholder:pl-4 bg-white outline-slate-300" name="search" placeholder="Rechercher par ville ou quartier">
        ...
        <div class="justify-center border-gray-500 border-2">
            ...
            @foreach ($ferronniers as $ferronnier)
                <a href="/vitriers/descriptions">
                    <div class="flex h-24 bg-slate-200 mb-2 items-center  m-2">
                        <div class="">
                            <img class="h-24 w-24 object-cover object-center" src="{{asset('storage/'.$ferronnier->photo)}}">
                        </div>
                        <div class="p-4">
                            <p><?= $ferronnier->lastName ?> <?= $ferronnier->firstName ?> </p>
                            <p ><?= $ferronnier->quarter ?> <?= $ferronnier->city ?> </p>
                        </div>
                    </div>
                </a>
            @endforeach
            <div class="flex flex-col items-center">
                <p>{{ $ferronniers->links() }}</p>
            </div>
        </div>
    </section>
    <x-footer/>
