...
<body class="box-border bg-blue-50">
    ...
    <section class="max-h-full px-44 mx-44">
        <div>
            <form action="{{url('/personnels/electriciens/search')}}" method="Get">
                @csrf
            ...
            @foreach ($electriciens as $electricien)
                <a href="/vitriers/descriptions">
                    <div class="flex h-24 bg-slate-200 mb-2 items-center  m-2">
                        <div class="">
                            <img class="h-24 w-24 object-cover object-center" src="{{asset('storage/'.$electricien->photo)}}">
                        </div>
                        <div class="p-4">
                            <p><?= $electricien->lastName ?> <?= $electricien->firstName ?> </p>
                            <p ><?= $electricien->quarter ?> <?= $electricien->city ?> </p>
                        </div>
                    </div>
                </a>
            @endforeach
            <div class="flex flex-col items-center">
                <p>{{ $electriciens->links() }}</p>
            </div>
        </div>
    </section>
    <x-footer/>
