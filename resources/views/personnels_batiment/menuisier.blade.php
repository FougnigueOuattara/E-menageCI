...
<body class="box-border bg-blue-50">
    ...
    <section class="max-h-full px-44 mx-44">
        <div>
            <form action="{{url('/personnels/menuisiers/search')}}" method="Get">
                @csrf
        ...
        <div class="justify-center border-gray-500 border-2">
            ...
            @foreach ($menuisiers as $menuisier)
                <a href="/vitriers/descriptions">
                    <div class="flex h-24 bg-slate-200 mb-2 items-center  m-2">
                        <div class="">
                            <img class="h-24 w-24 object-cover object-center" src="{{asset('storage/'.$menuisier->photo)}}">
                        </div>
                        <div class="p-4">
                            <p><?= $menuisier->lastName ?> <?= $menuisier->firstName ?> </p>
                            <p ><?= $menuisier->quarter ?> <?= $menuisier->city ?> </p>
                        </div>
                    </div>
                </a>
            @endforeach
            <div class="flex flex-col items-center">
                <p>{{ $menuisiers->links() }}</p>
            </div>
        </div>
    </section>
    <x-footer/>
