...
<body class="box-border bg-blue-50">
    ...
    <section class="max-h-full px-44 mx-44">
        <div>
            <form action="{{url('/personnels/peintres/search')}}" method="Get">
                @csrf
                <div class="flex justify-between pb-4 pt-4">
                    <input class="rounded-sm h-10 w-2/3 pl-2 placeholder:pl-4 bg-white outline-slate-300" name="search" placeholder="Rechercher par ville ou quartier">
        ...
        <div class="justify-center border-gray-500 border-2">
            ...
            @foreach ($peintres as $peintre)
                <a href="/vitriers/descriptions">
                    <div class="flex h-24 bg-slate-200 mb-2 items-center  m-2">
                        <div class="">
                            <img class="h-24 w-24 object-cover object-center" src="{{asset('storage/'.$peintre->photo)}}">
                        </div>
                        <div class="p-4">
                            <p><?= $peintre->lastName ?> <?= $peintre->firstName ?> </p>
                            <p ><?= $peintre->quarter ?> <?= $peintre->city ?> </p>
                        </div>
                    </div>
                </a>
            @endforeach
