...
<body class="box-border bg-blue-50">
    ...
    <section class="max-h-full px-44 mx-44">
        <div>
            <form action="{{url('/personnels/vitriers/search')}}" method="Get">
                @csrf
        ...
        <div class="justify-center border-gray-500 border-2">
            ...
            @foreach ($vitriers as $vitrier)
                <a href="/vitriers/descriptions">
                    <div class="flex h-24 bg-slate-200 mb-2 items-center  m-2">
                        <div class="">
                            <img class="h-24 w-24 object-cover object-center" src="{{asset('storage/'.$vitrier->photo)}}">
                        </div>
                        <div class="p-4">
                            <p><?= $vitrier->lastName ?> <?= $vitrier->firstName ?> </p>
                            <p ><?= $vitrier->quarter ?> <?= $vitrier->city ?> </p>
                        </div>
                    </div>
                </a>
            @endforeach
