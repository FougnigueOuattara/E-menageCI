...
<body class="box-border bg-blue-50">
    ...
    <section class="max-h-full px-44 mx-44">
        <div>
            <form action="{{url('/personnels/menagers/search')}}" method="Get">
                @csrf
            ...
            @foreach ($menagers as $menager)
                <a href="/vitriers/descriptions">
                    <div class="flex h-24 bg-slate-200 mb-2 items-center  m-2">
                        <div class="">
                            <img class="h-24 w-24 object-cover object-center" src="{{asset('storage/'.$menager->photo)}}">
                        </div>
                        <div class="p-4">
                            <p><?= $menager->lastName ?> <?= $menager->firstName ?> </p>
                            <p ><?= $menager->quarter ?> <?= $menager->city ?> </p>
                        </div>
                    </div>
                </a>
            @endforeach
