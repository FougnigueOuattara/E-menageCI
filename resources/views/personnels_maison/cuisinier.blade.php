...
<body class="box-border bg-blue-50">
    ...
    <section class="max-h-full px-44 mx-44">
        <div>
            <form action="{{url('/personnels/cuisiniers/search')}}" method="Get">
                @csrf
                <div class="flex justify-between pb-4 pt-4">
                    <input class="rounded-sm h-10 w-2/3 pl-2 bg-white outline-slate-300" name="search" placeholder="Rechercher par ville ou quartier">
            ...
            @foreach ($cuisiniers as $cuisinier)
                <a href="/vitriers/descriptions">
                    <div class="flex h-24 bg-slate-200 mb-2 items-center  m-2">
                        <div class="">
                            <img class="h-24 w-24 object-cover object-center" src="{{asset('storage/'.$cuisinier->photo)}}">
                        </div>
                        <div class="p-4">
                            <p><?= $cuisinier->lastName ?> <?= $cuisinier->firstName ?> </p>
                            <p ><?= $cuisinier->quarter ?> <?= $cuisinier->city ?> </p>
                        </div>
                    </div>
                </a>
            @endforeach
            <div class="flex flex-col items-center">
                <p>{{ $cuisiniers->links() }}</p>
            </div>
        </div>
    </section>
    <x-footer/>
